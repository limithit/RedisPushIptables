...
```
The parameter `timeout` has the same effect as `ttl.drop.insert`. If a `timeout` is configured, ipset is used to implement periodic deletion. If the `timeout` is not configured, it is periodically removed using `ttl.drop.insert`.
