...
```
The `timeout` parameter has the same effect as `ttl.drop.insert` parameter. If `timeout` parameter is configured, ipset is used to implement periodic deletion. If the `timeout` parameter is not configured, it is periodic deletion used `ttl.drop.insert`.
